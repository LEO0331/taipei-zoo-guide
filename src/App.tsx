...
const formatTableCoordinate = (value: number | null) => value?.toFixed(4) ?? '';
...
function RiverfrontBirdGuide({ records, search, language, onSelect }: { records: RiverfrontBirdObservation[]; search: string; language: Language; onSelect: (record: RiverfrontBirdObservation) => void }) {
  ...
  return <>
    ...
    <div className="table-wrap"><table className="observation-table" data-testid="riverfront-bird-table"><thead><tr>{[zh?'中文名':'Chinese name',zh?'學名':'Scientific name',zh?'科':'Family',zh?'狀態':'Source status',zh?'觀察期間':'Period',zh?'河域':'Region',zh?'數量':'Count', 'TWD97 X','TWD97 Y',zh?'經度':'Longitude',zh?'緯度':'Latitude'].map(x=><th key={x}>{x}</th>)}</tr></thead><tbody>{filtered.slice(0,200).map(r=><tr key={r.id} onClick={()=>onSelect(r)}><td>{r.commonNameZh}</td><td><em>{r.scientificName}</em></td><td>{r.familyName}</td><td>{r.statusRaw}</td><td>{r.observationPeriod}{r.datePrecision==='month'?' (month only)':''}</td><td>{r.region}</td><td>{r.observedCount}</td><td>{r.xTwd97Raw}</td><td>{r.yTwd97Raw}</td><td>{formatTableCoordinate(r.longitude)}</td><td>{formatTableCoordinate(r.latitude)}</td></tr>)}</tbody></table></div>
  </>;
}
...
function RiverfrontReptileGuide({ records, search, language, onSelect }: { records: RiverfrontReptileObservation[]; search: string; language: Language; onSelect: (record: RiverfrontReptileObservation) => void }) {
  ...
  return <>
    ...
    <div className="table-wrap"><table className="observation-table" data-testid="riverfront-reptile-table"><thead><tr>{[zh ? 'ID' : 'ID', zh ? '類群' : 'Group', zh ? '科' : 'Family', zh ? '中文名' : 'Chinese name', zh ? '學名' : 'Scientific name', zh ? '期間' : 'Period', zh ? '時間' : 'Time', zh ? '河域' : 'Region', zh ? '數量' : 'Count', 'TWD97 X', 'TWD97 Y', zh ? '經度' : 'Longitude', zh ? '緯度' : 'Latitude'].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{filtered.slice(0, 200).map((record) => <tr key={record.id} onClick={() => onSelect(record)}><td>{record.sourceSequenceNumber}</td><td>{record.group}</td><td>{record.familyName}</td><td>{record.commonNameZh}</td><td><em>{record.scientificName}</em></td><td>{record.observationPeriod}</td><td>{record.observationTime}</td><td>{record.region}</td><td>{record.observedCount}</td><td>{record.xTwd97Raw}</td><td>{record.yTwd97Raw}</td><td>{formatTableCoordinate(record.longitude)}</td><td>{formatTableCoordinate(record.latitude)}</td></tr>)}</tbody></table></div>
  </>;
}
...
function GuideMap({
  ...
  return (
    <section className="map-section">
      ...
      <div className="map-shell" data-testid="guide-map">
