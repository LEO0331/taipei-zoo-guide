...
- 地圖底圖改用 OpenStreetMap 圖磚，並保留必要的貢獻者標示；不需要商業地圖 API 金鑰。
...
資料會依分頁按需載入。首次開啟只會載入動物導覽；大型生物多樣性與河濱觀察資料會在進入對應分頁時才讀取，地圖中的歷史野生動物圖層則在使用者啟用後才載入。Service Worker 對資料採網路優先、離線備援快取：在線時可取得更新後的產生資料，也不會在安裝時下載全部紀錄。
